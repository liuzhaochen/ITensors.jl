# QN ITensor complex contraction benchmark: heap vs buffer, single vs threaded
# Tests ComplexF64 block-sparse contraction with buffer allocation.
#
# Usage:
#   julia -t 4 --project=benchmark benchmark/qn_itensor_complex_benchmark.jl
...
println("="^70)
println("QN ITensor Complex Contraction Benchmark (BenchmarkTools)")
println("="^70)
...
const A_HEAP = random_itensor(ComplexF64, QN(0), i, j)
const B_HEAP = random_itensor(ComplexF64, QN(0), dag(j), k)

println("  A nnz: $(nnz(A_HEAP)) / $(dim(i)*dim(j)) elements (ComplexF64)")
println("  B nnz: $(nnz(B_HEAP)) / $(dim(j)*dim(k)) elements (ComplexF64)")
...
function bench_heap_seq()
    ...
    B = random_itensor(ComplexF64, QN(0), dag(j), k)
    A * B
end
